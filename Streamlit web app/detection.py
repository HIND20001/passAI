...
def recommend_passes(player_positions, ball_holder_position, player_team_indices, goal_position, tac_map_copy):
    # Créer un graphe pondéré pour les passes
    G = nx.Graph()
    
    # Ajouter les noeuds (joueurs) au graphe
    for i, pos in enumerate(player_positions):
        G.add_node(i, pos=pos)
    
    # Ajouter les arêtes (distances) entre les joueurs du même équipe
    for i in range(len(player_positions)):
        for j in range(i + 1, len(player_positions)):
            if player_team_indices[i] == player_team_indices[j]:
                dist = distance.euclidean(player_positions[i], player_positions[j])
                G.add_edge(i, j, weight=dist)
    
    # Identifier le joueur le plus proche de la position du ballon
    ball_holder_index = None
    min_distance = float('inf')
    
    for i, pos in enumerate(player_positions):
        dist = distance.euclidean(pos, ball_holder_position)
        if dist < min_distance:
            min_distance = dist
            ball_holder_index = i
    
    if ball_holder_index is None:
        raise ValueError("Ball holder position not found in player positions")
    
    # Trouver les chemins les plus courts vers chaque joueur de l'équipe
    shortest_paths = nx.shortest_path(G, source=ball_holder_index, weight='weight')
    
    best_path = None
    best_goal_path_length = float('inf')
    
    # Rechercher le meilleur chemin vers le but
    for target in G.nodes:
        path_length = nx.dijkstra_path_length(G, source=ball_holder_index, target=target, weight='weight')
        target_position = G.nodes[target]['pos']
        if path_length < best_goal_path_length and distance.euclidean(target_position, goal_position) < 100:
            best_path = nx.dijkstra_path(G, source=ball_holder_index, target=target, weight='weight')
            best_goal_path_length = path_length
    
    passes = []
    if best_path:
        for i in range(len(best_path) - 1):
            start_pos = G.nodes[best_path[i]]['pos']
            end_pos = G.nodes[best_path[i + 1]]['pos']
            tac_map_copy = cv2.line(tac_map_copy, start_pos, end_pos, (0, 255, 0), 2)
            passes.append((start_pos, end_pos))
    
    return passes, tac_map_copy
...
def transform_points(points, homography_matrix):
    ...
    for pt in points:
        ...
        transformed_pt = np.dot(homography_matrix, pt_homogeneous)
        transformed_pt /= transformed_pt[2]
        transformed_points.append((int(transformed_pt[0]), int(transformed_pt[1])))
...
def create_colors_info(team1_name, team1_p_color, team1_gk_color, team2_name, team2_p_color, team2_gk_color):
    ...
    color_list_lab = [skimage.color.rgb2lab(np.array(c) / 255) for c in colors_list]  # Converting color_list to L*a*b* space
        ...
        if success:
            ...
            if 'homog' in locals():
                ...
                for pt in detected_ppos_src_pts:  # Loop over players frame coordinates
                    ...
                    dest_point = np.matmul(homog, np.transpose(pt))  # Apply homography transformation
            ...
            palette_interval = (0, num_pal_colors)  # Color interval to extract from dominant colors palette (1st to 5th color)
            ...
            for palette in obj_palette_list:
                palette_distance = []
                palette_lab = [skimage.color.rgb2lab(np.array(color) / 255) for color in palette]  # Convert colors to L*a*b* space
                ...
                if labels_p[i] == 0:  # Display annotation for detected players (label 0)
                    ...
                    if show_pal:
                        ...
                        for k, c in enumerate(palette):
                            ...
                            annotated_frame = cv2.rectangle(annotated_frame, (int(bboxes_p[i, 2]) + 3, int(bboxes_p[i, 1]) + k * palette_box_size),
                    ...
                    if detected_ball_src_pos is not None and 'homog' in locals():
                        tac_map_copy = cv2.circle(tac_map_copy, (int(detected_ball_dst_pos[0]), int(detected_ball_dst_pos[1])), radius=5,
